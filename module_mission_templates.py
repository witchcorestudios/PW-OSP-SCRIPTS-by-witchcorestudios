...
before_mission_start_setup = (ti_before_mission_start, 0, 0, [], # set up basic mission and scene values
   [(server_set_friendly_fire, 1),
	...
	(assign, "$antimassbanticker4", 0),
	(assign, "$antimassbanticker5", 0),
	(assign, "$antimassbanid1", 0),
	(assign, "$antimassbanid2", 0),
	(assign, "$antimassbanid3", 0),
	(assign, "$antimassbanid4", 0),
...
player_joined = (ti_server_player_joined, 0, 0, [], # server: handle connecting players
   [(store_trigger_param_1, ":player_id"),
    (call_script, "script_cf_add_player", ":player_id"),
#   (call_script, "script_player_check_name", ":player_id"),
	(player_set_slot, ":player_id", slot_player_changed_troop, 0),
    (call_script, "script_setup_player_joined", ":player_id"),
	(player_set_slot, ":player_id", slot_player_joined, 1),
	(player_set_slot, ":player_id", slot_player_disconnected_ok, 0),
	(call_script, "script_cf_server_check_admin", ":player_id"),
	(val_add, "$playersonline", 1),
    ])

player_exit = (ti_on_player_exit, 0, 0, [], # server: save player values on exit
   [(store_trigger_param_1, ":player_id"),
	(player_set_slot, ":player_id", slot_player_disconnected_ok, 1),
	(call_script, "script_cf_player_save_script", ":player_id"),
	(call_script, "script_cf_save_player_exit", ":player_id"),
	(val_sub, "$playersonline", 1),
	(player_set_slot, ":player_id", slot_player_changed_troop, 0),
#	(else_try),
#	(player_slot_eq, ":player_id", slot_player_killed_check, 1),
#	(call_script, "script_reset_player_on_death", ":agent_id"),

#	(else_try),
#	(player_slot_eq, ":player_id", slot_player_killed_check, 0),
    ])
...
agent_spawn = (ti_on_agent_spawn, 0, 0, [], # server and clients: set up new agents after they spawn
   [(store_trigger_param_1, ":agent_id"),
	...
	(player_slot_eq, ":player_id", slot_player_joined, 1),
	(agent_get_player_id, ":player_id", ":agent_id"),
	(player_slot_eq, ":player_id", slot_player_changed_troop, 0),
	(call_script, "script_cf_player_spawning_loop", ":agent_id"),
	(call_script, "script_update_name", ":agent_id"),
	(call_script, "script_cf_server_check_admin", ":player_id"),
...
agent_killed = (ti_on_agent_killed_or_wounded, 0, 0, [], # server and clients: handle messages, score, loot, and more after agents die
   [(store_trigger_param_1, ":dead_agent_id"),
	...
	(player_set_slot, ":player_id", slot_player_killed_check, 1),
    (try_begin),
	(agent_is_human, ":dead_agent_id"),
	(player_slot_eq, ":player_id", slot_player_disconnected_ok, 0),
	(call_script, "script_setup_agent_for_respawn", ":dead_agent_id"),
	(call_script, "script_reset_player_on_death", ":dead_agent_id"),
	...
	(else_try),
	(agent_is_human, ":dead_agent_id"),
    (player_slot_eq, ":player_id", slot_player_disconnected_ok, 1),
	(player_set_slot, ":player_id", slot_player_killed_check, 0),
	(try_end),
    ])
...
(try_begin), # check if damage should bleed through the armor due to unmet requirements
	...
	(try_end),
	(call_script, "script_cf_hit_logging", ":attacked_agent_id", ":attacker_agent_id", ":damage_dealt"),
    ])
...
agent_check_loop = (0, 0, 0.5, # server: loop over all agents, doing all common repetitive checks together for each agent, to minimize the penalty of using try_for_agents
   [(multiplayer_is_server),
    (try_begin), # if the loop was not restarted
	  ...
	  (try_begin),
	  (gt, ":agent_id", 0),
	  (val_add, "$factionbonusticker", 1),
	  (ge, "$factionbonusticker", 300),
	  (ge, "$playersonline", 5),
	  (agent_get_player_id, ":player_id", ":agent_id"),
	  (player_is_active, ":player_id"),
	  (player_get_slot, ":faction_id", ":player_id", slot_player_faction_id),
	  (gt, ":faction_id", 1),
	  (call_script, "script_player_adjust_gold", ":player_id", "$factionbonus", 500),
      (str_store_string, s11, "@Faction Bonus Recieved [!]"),
...
#agent_get_faction_bonus = (180, 0, 0, [], # server: repeatedly check all agents for attacking with a weapon they can't use - should be kept as simple as possible
#   [(multiplayer_is_server),
#      (try_for_agents, ":agent_id"),
#	  (agent_get_player_id, ":player_id", ":agent_id"),
#	  (player_get_slot, ":faction_id", ":player_id", slot_player_faction_id),
#	  (gt, ":faction_id", 1),
#	  (call_script, "script_player_adjust_gold", ":player_id", "$factionbonus", 1),
#      (str_store_string, s11, "@Faction Bonus Recieved [!]"),
#      (multiplayer_send_string_to_player, ":player_id", server_event_local_chat, s11),
#	  (try_end),
#    ])
	
anti_mass_ban = (60, 0, 0, [], # server: repeatedly check all agents for attacking with a weapon they can't use - should be kept as simple as possible
   [(multiplayer_is_server),
    (try_begin),
	(ge, "$antimassbanticker1", 3),
	(call_script, "script_mass_ban_protection", "$antimassbanid1"),
	(else_try),
	(assign, "$antimassbanticker1", 0),
    (try_end), 
	(try_begin),
	(ge, "$antimassbanticker2", 3),
	(call_script, "script_mass_ban_protection", "$antimassbanid2"),
	(else_try),
	(assign, "$antimassbanticker2", 0),
    (try_end), 
	(try_begin),
	(ge, "$antimassbanticker3", 3),
	(call_script, "script_mass_ban_protection", "$antimassbanid3"),
	(else_try),
	(assign, "$antimassbanticker3", 0),
    (try_end), 
	(try_begin),
	(ge, "$antimassbanticker4", 3),
	(call_script, "script_mass_ban_protection", "$antimassbanid4"),
	(else_try),
	(assign, "$antimassbanticker4", 0),
    (try_end), 
	(try_begin),
	(ge, "$antimassbanticker5", 3),
	(call_script, "script_mass_ban_protection", "$antimassbanid5"),
	(else_try),
	(assign, "$antimassbanticker5", 0),
	(try_end), 
    ])
